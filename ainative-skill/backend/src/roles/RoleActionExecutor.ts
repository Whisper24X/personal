...
export class RoleActionExecutor {
  ...
  private prepareActionInput(action: BaseAction): string {
    ...
    const workspaceReadActions = ['TestReview', 'AutomationPlanning', 'AutomationExecution', 'ApiAutomationPlanning', 'ApiAutomationExecution'];
  ...
  private prepareReviewInput(actionName: string, docType: string): string {
    ...
    logger.warn(`${this.profile} ${docType}Review: No ${docType} found in news or memory, will try to read from workspace`);
    return '';
  }
  ...
  private prepareImproveInput(reviewActionName: string, reviewType: string): string {
    ...
    logger.warn(`${this.profile} Improve: No ${reviewType} found in news or memory, will try to read from workspace`);
    return '';
  }
   ...
   */
  private async executeAction(action: BaseAction, input: string, workspaceOptions: WorkspaceOptions | undefined): Promise<any> {
   ...
   */
  private async runActionWithOptions(action: BaseAction, input: string, workspaceOptions: WorkspaceOptions | undefined): Promise<any> {
    const actionName = action.name;
    ...
    switch (actionName) {
      ...
      case 'ApiAutomationPlanning':
      case 'ApiAutomationExecution':
  ...
  private handleSequenceContinuation(): void {
    const hasMoreActions = this.rc.reactMode === RoleReactMode.BY_ORDER && this.rc.state >= 0 && this.rc.state < this.actions.length - 1;
