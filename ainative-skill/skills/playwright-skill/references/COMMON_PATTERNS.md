...
- [Idempotent Dropdown Selection](#idempotent-dropdown-selection)
- [Element Plus Form Element Locators](#element-plus-form-element-locators)
...
## Element Plus Form Element Locators

When TEST.md uses path-guide format (e.g. "在搜索区域点击 label 为「购买渠道」的下拉框"):

- **el-form-item + el-select**: `page.locator('.el-form-item').filter({ hasText: '购买渠道' }).locator('.el-select').first()`
- **Dropdown options**: After clicking the select, use `page.locator('.el-select-dropdown:visible, .el-popper:visible').getByText('其他')` — **avoid `.last()`** as it is fragile when multiple poppers exist
