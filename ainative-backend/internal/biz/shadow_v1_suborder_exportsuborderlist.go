...
func (s *ShadowV1SubOrderUseCase) ExportSubOrderCsv(ctx context.Context, subOrderList *pb.GetSubOrderListReply, filePath string) error {
	...
	// GetSubOrderList 已经返回了 GoodName 和 ChannelName，直接使用即可

	// 写入数据
	for _, v := range subOrderList.List {
		// 格式化金额（分转元）
		...
		refundAmountYuan := float32(v.RefundAmount) / 100.0
...
// ExportSubOrderList 子订单-导出子订单信息
func (s *ShadowV1SubOrderUseCase) ExportSubOrderList(ctx context.Context, req *pb.ExportSubOrderListReq) (*pb.ExportSubOrderListReply, error) {
	resp := &pb.ExportSubOrderListReply{}

	// 循环查询所有数据，每次查询3000条
	var allSubOrders []*pb.SubOrderInfo
	page := int32(1)
	pageSize := int32(3000)

	for {
		reply, err := s.GetSubOrderList(ctx, &pb.GetSubOrderListReq{
			Page:             page,
			PageSize:         pageSize,
			...
			CampTimeStart:    req.GetCampTimeStart(),
			CampTimeEnd:      req.GetCampTimeEnd(),
		})
		if err != nil {
		...
		// 将当前页的数据追加到总列表中
		if len(reply.List) > 0 {
			allSubOrders = append(allSubOrders, reply.List...)
		}

		// 如果返回的数据少于pageSize，说明已经查询完所有数据
		if len(reply.List) < int(pageSize) {
			break
		}
	...
	dirPath := filepath.Join(currentDir, "yanxue-export", "sub_order")
	// 检查目录是否存在，不存在则创建
	...
	// 生成csv文件
	err = s.ExportSubOrderCsv(ctx, reply, path)
	if err != nil {
		return resp, err
	}
