 ...
 * and flags eval reports when diff/size or secrets exceed limits. Maintains an in-memory record ring buffer.
 */
export class DefaultObservabilityGovernancePlugin implements ObservabilityGovernancePlugin {
  ...
  /**
   * Post-eval: sanitize report, derive diff metrics, and flag secret/risk findings for downstream PR reporting.
   */
  async afterEval(result: EvalResult): Promise<void> {
    ...
    if (diff.filesChanged > this.policy.maxFilesChanged || diff.changedLines > this.policy.maxDiffLines) {
      ...
      outcome = "flagged";
      message = "Governance flagged evaluation because diff risk exceeded policy";
    }
    result.report = {
      ...result.report,
