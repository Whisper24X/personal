...
describe("SQLite repositories integration", () => {
  ...
  beforeAll(async () => {
    workspaceDir = mkdtempSync(join(tmpdir(), "diting-sqlite-repos-"));
  ...
  it("supports task and agent claim semantics against SQLite", async () => {
    ...
    const readyTask = createTask({ status: "ready" });
    const secondTask = {
      ...createTask(),
      id: "task-2",
      externalId: "EXT-2",
    ...
    await tasks.create(readyTask);
    await tasks.create(secondTask);
    await agents.upsert(idleAgent);

    const claimedTask = await tasks.claimQueued(readyTask.id, now);
    const missedTask = await tasks.claimQueued(readyTask.id, now);
    const claimedAgent = await agents.claimIdle("codex", readyTask.id, now);
  ...
  it("round-trips RunAttempt records and reads WaitReason metadata", async () => {
    const tasks = new PgTaskRepository(pool);
    const runAttempts = new PgRunAttemptRepository(pool);
    ...
    await tasks.create(waitingTask);
    await tasks.create(activeTask);
    await runAttempts.create(attempt);

    const fetchedAttempt = await runAttempts.getLatestByTask(activeTask.id);
    const fetchedWaiting = await tasks.getById(waitingTask.id);

    expect(fetchedAttempt).toEqual(expect.objectContaining({
      taskId: activeTask.id,
      stage: "evaluating",
      agentId: "agent-1"
    }));
    expect(readWaitReason(fetchedWaiting?.metadata ?? {})).toEqual(
      expect.objectContaining({
        type: "human_input",
        message: "Waiting for operator"
      })
  ...
  it("claims only the specified idle agent by id", async () => {
    const agents = new PgAgentRepository(pool);
    const now = new Date("2026-05-11T00:05:00.000Z");
    const firstAgent = {
      ...createAgent(),
    ...
    await agents.upsert(firstAgent);
    await agents.upsert(secondAgent);

    const claimed = await agents.claimIdleById(secondAgent.id, "task-2", now);
    const missed = await agents.claimIdleById(secondAgent.id, "task-3", now);
    const untouched = await agents.getById(firstAgent.id);
...
async function truncateAll(pool: Awaited<ReturnType<typeof createDatabase>>["pool"]): Promise<void> {
  ...
  await pool.query("delete from run_attempts");
