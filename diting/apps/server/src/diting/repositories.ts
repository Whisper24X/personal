...
/**
 * SQLite 上的仓储实现：SQL 与类型名保留 `Pg*` 前缀以对齐 `@diting/plugin-api` 契约与既有迁移。
...
export class PgTaskRepository implements TaskRepository {
  ...
  async list(query: TaskListQuery = {}): Promise<TitingTask[]> {
    ...
    return result.rows
      .map(mapTask)
      .filter((task) => (query.agentKind ? task.agentKind === query.agentKind : true));
  }
...
export class PgRunAttemptRepository implements RunAttemptRepository {
  constructor(private readonly pool: DatabaseClient) {}
  ...
  async save(attempt: RunAttempt): Promise<void> {
    await this.pool.query(
      `insert into run_attempts (
        id, task_id, agent_id, stage, release_reason, metadata_json, started_at, ended_at
      ...
      [
        ...
        attempt.agentId,
        attempt.stage,
        attempt.releaseReason ?? null,
        JSON.stringify(encodeJsonObject(attempt.metadata)),
        attempt.startedAt,
  ...
  async getById(id: string): Promise<RunAttempt | null> {
    const result = await this.pool.query("select * from run_attempts where id = $1", [id]);
    return result.rows[0] ? mapRunAttempt(result.rows[0]) : null;
  }

  async getLatestByTask(taskId: string): Promise<RunAttempt | null> {
    const result = await this.pool.query(
      "select * from run_attempts where task_id = $1 order by started_at desc, id desc limit 1",
      [taskId]
    );
    return result.rows[0] ? mapRunAttempt(result.rows[0]) : null;
  }
  ...
  async list(query: RunAttemptListQuery = {}): Promise<RunAttempt[]> {
    ...
    const where = clauses.length > 0 ? `where ${clauses.join(" and ")}` : "";
    const result = await this.pool.query(
      `select * from run_attempts ${where} order by started_at desc, id desc limit $${values.length}`,
      values
    );
    return result.rows.map(mapRunAttempt);
  }

  async listByTask(taskId: string): Promise<RunAttempt[]> {
    const result = await this.pool.query(
      "select * from run_attempts where task_id = $1 order by started_at desc, id desc",
      [taskId]
    );
    return result.rows.map(mapRunAttempt);
  }
...
/** Read current WaitReason from task metadata envelope or legacy plain object. */
export function readWaitReason(metadata: Record<string, unknown>): WaitReason | null {
  const candidate = metadata.waitReason;
  ...
  return {
    type: value.type as WaitReason["type"],
    source: typeof value.source === "string" ? value.source : "unknown",
    message: value.message,
    externalRef: typeof value.externalRef === "string" ? value.externalRef : undefined,
    recoverableBy: (typeof value.recoverableBy === "string" ? value.recoverableBy : "user") as WaitReason["recoverableBy"],
    createdAt: typeof value.createdAt === "string" ? value.createdAt : new Date().toISOString()
  };
}

/** Map legacy persisted task status values to the new lifecycle model. */
export function mapLegacyTaskStatus(value: unknown): TitingTask["status"] {
...
/** Infer RunAttempt stage when backfilling legacy in-flight task statuses. */
export function inferLegacyAttemptStage(status: unknown): RunAttempt["stage"] {
...
export class PgExecutionRepository implements ExecutionRepository {
  ...
  async getById(id: string): Promise<ExecutionRecord | null> {
    const result = await this.pool.query("select * from executions where id = $1", [id]);
    return result.rows[0] ? mapExecution(result.rows[0]) : null;
  }

  async list(query: ExecutionListQuery = {}): Promise<ExecutionRecord[]> {
    const values: unknown[] = [];
    const clauses: string[] = [];
    if (query.taskId) {
      values.push(query.taskId);
    ...
    const where = clauses.length > 0 ? `where ${clauses.join(" and ")}` : "";
    const result = await this.pool.query(
      `select * from executions ${where} order by started_at desc, id desc limit $${values.length}`,
      values
    );
  ...
  async upsert(goal: RepairGoal): Promise<void> {
    await this.pool.query(
      `insert into repair_goals (
        ...
        last_failure_hash, metadata_json, created_at, updated_at
      ...
      [
        ...
        JSON.stringify(encodeJsonObject(goal.metadata)),
        goal.createdAt,
        goal.updatedAt
      ]
    );
...
export class PgAgentLeaseRepository implements AgentLeaseRepository {
  ...
  async listByExecution(executionId: string): Promise<AgentLease[]> {
    const result = await this.pool.query(
      "select * from agent_leases where execution_id = $1 order by leased_at desc",
      [executionId]
    );
    return result.rows.map(mapAgentLease);
  }
...
function mapTask(row: Record<string, unknown>): TitingTask {
  const metadata = decodeJsonObject(row.metadata_json);
  const agentRequest = readAgentRequest(metadata);
  const executor = String(row.executor);
  return {
    id: String(row.id),
    ...
    executor,
    agentKind: agentRequest.agentKind ?? inferAgentKindFromExecutor(executor),
    preferredDriver: agentRequest.preferredDriver ?? inferPreferredDriver(executor),
    preferredRuntime: agentRequest.preferredRuntime ?? inferPreferredRuntime(executor),
    driverId: agentRequest.preferredDriver ?? inferPreferredDriver(executor),
    runtimeProviderId: agentRequest.preferredRuntime ?? inferPreferredRuntime(executor),
...
function mapExecution(row: Record<string, unknown>): ExecutionRecord {
  ...
  return {
    ...
    agentKind: inferAgentKindFromExecutor(executor),
    driverId: inferPreferredDriver(executor),
    runtimeProviderId: inferPreferredRuntime(executor),
...
function mapAgent(row: Record<string, unknown>): AgentRecord {
  ...
  return {
    ...
    executor,
    kind: inferAgentKindFromExecutor(executor),
    driverId: inferPreferredDriver(executor),
    runtimeProviderId: inferPreferredRuntime(executor),
...
function mapRepairGoal(row: Record<string, unknown>): RepairGoal {
  return {
    ...
    metadata: row.metadata_json === undefined ? {} : decodeJsonObject(row.metadata_json),
    createdAt: new Date(String(row.created_at)),
    updatedAt: new Date(String(row.updated_at))
  };
}
...
function mapRunAttempt(row: Record<string, unknown>): RunAttempt {
  return {
    id: String(row.id),
    taskId: String(row.task_id),
    agentId: String(row.agent_id ?? ""),
    stage: row.stage as RunAttempt["stage"],
...
function readAgentRequest(metadata: Record<string, unknown>): {
  agentKind?: AgentKind;
  preferredDriver?: string;
  preferredRuntime?: string;
} {
  const candidate = metadata.agentRequest;
  if (!candidate || typeof candidate !== "object" || Array.isArray(candidate)) {
    return {};
  }
  const value = candidate as Record<string, unknown>;
  return {
    agentKind: typeof value.agentKind === "string" && value.agentKind.trim() ? (value.agentKind.trim() as AgentKind) : undefined,
    preferredDriver: typeof value.preferredDriver === "string" && value.preferredDriver.trim() ? value.preferredDriver.trim() : undefined,
    preferredRuntime: typeof value.preferredRuntime === "string" && value.preferredRuntime.trim() ? value.preferredRuntime.trim() : undefined
  };
}

function inferAgentKindFromExecutor(executor: string): AgentKind {
  if (executor === "codex" || executor === "cursor" || executor === "programming") {
    return "programming";
  }
  return executor;
}

function inferPreferredDriver(executor: string): string | null {
  return inferAgentKindFromExecutor(executor) === "programming" ? "coding" : null;
}

function inferPreferredRuntime(executor: string): string | null {
  if (executor === "codex" || executor === "cursor") {
    return executor;
  }
  return null;
}
