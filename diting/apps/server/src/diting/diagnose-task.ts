...
export function buildDiagnosis(input: DiagnosisInput) {
  ...
  const taskMetadata = decodeJsonObject(input.task.metadata_json);
  const productReview = decodeJsonObject(taskMetadata.openSpecReview);
  const openspecPath = readStringValue(productReview.openspecPath) ?? readStringValue(taskMetadata.openspecPath);
  const qualityArtifacts = readQualityArtifacts(taskMetadata);
  ...
  return {
    summary: {
      ...
      agentKind: input.task.agent_kind ? String(input.task.agent_kind) : null,
      driverId: input.task.driver_id ? String(input.task.driver_id) : null,
      runtimeProviderId: input.task.runtime_provider_id ? String(input.task.runtime_provider_id) : null,
    ...
    inferredFailureMode: {
      ...
      productReview: Object.keys(productReview).length > 0 ? {
        status: String(input.task.status) === "waiting" ? "waiting_for_review" : "available",
        externalId: productReview.externalId ? String(productReview.externalId) : null,
        lastDecision: productReview.lastDecision ? String(productReview.lastDecision) : null,
        error: productReview.error ? String(productReview.error) : null,
        openspecPath
      } : null,
      qualityArtifacts,
...
function readQualityArtifacts(metadata: Record<string, unknown>) {
  const implementationHandoffPath = readStringValue(metadata.implementationHandoffPath);
  const qualityReportPath = readStringValue(metadata.qualityReportPath);
  const qualityRepairHandoffPath = readStringValue(metadata.qualityRepairHandoffPath);
  if (!implementationHandoffPath && !qualityReportPath && !qualityRepairHandoffPath) {
    return null;
  }
  return {
    implementationHandoffPath,
    qualityReportPath,
    qualityRepairHandoffPath
  };
}
