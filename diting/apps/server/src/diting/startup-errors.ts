...
/** `fastify.listen` 抛出的端口占用错误（Node ErrnoException，附带 port）。 */
...
/** 顶层 catch：沿用 DB 结构化错误，端口占用给出明确提示，否则包一层通用 bootstrap 文案。 */
export function wrapBootstrapError(error: unknown): Error {
  ...
  if (isAddressInUseError(error)) {
    ...
    return new Error(
      `[server:listen] 端口 ${port} 已被占用，服务无法启动。请先释放该端口（lsof -i :${port} 查看占用进程），或通过环境变量 BACKEND_PORT 改用其他端口`,
      { cause: error }
    );
  }
