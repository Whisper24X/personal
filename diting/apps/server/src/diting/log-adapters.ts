/**
 * 将 `@diting/plugin-api` 可观测事件、执行日志落盘到当前选中的 `LogPlugin`，并在需要时拆出
...
export class FileExecutionLogRepository implements ExecutionLogRepository {
  ...
  async listByExecution(executionId: string): Promise<ExecutionLogRecord[]> {
    const plugin = this.plugin as LogPlugin & {
      listByExecution?: (executionId: string) => Promise<ExecutionLogRecord[]>;
    };
    if (typeof plugin.listByExecution === "function") {
      return plugin.listByExecution(executionId);
    }
    return [];
  }
}
