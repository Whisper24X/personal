...
async function main() {
  ...
  // 默认有界面：启动时清空继承的 HEADLESS；仅 CI 或 PLAYWRIGHT_HEADLESS=1 时无头（忽略误设的 HEADLESS=true）
  const forceHeaded = process.env.PLAYWRIGHT_FORCE_HEADED === '1';
  const wantHeadless =
    !forceHeaded &&
  ...
  try {
    ...
    // 脚本若以 module.exports = (async () => { ... })() 导出 Promise，必须 await，否则主流程不等待自动化结束
