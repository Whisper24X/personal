# diting Technical Design

## Overview

diting is an AI engineering execution controller. It controls task lifecycle, state transitions, scheduling, Goal Loop execution, observability, and governance. It does not directly execute coding tools or infrastructure steps. Those capabilities are delegated to plugins.
...
- SQLite via bundled node:sqlite binding; default file .diting/sqlite/diting.sqlite, overridable via env DATABASE_FILE
- SQL migrations
...
- Convert remote fields to ditingTask
- Report results back
...
质量闭环已从普通编程执行链路中拆分为独立 `quality` agent kind。编码 Agent 完成实现后，不再直接在同一次编程执行内收敛质量检查，而是写入 `artifacts/implementation-handoff.json`，并通过任务流转把任务交给 `quality-orchestrator`。

`quality-orchestrator` 的边界是阶段编排，不替代底层检查能力：

- `CompletionGatePlugin` 负责 OpenSpec 完成度判定。
- `QualityPlugin` 负责 lint、typecheck、unit test、build、diff risk 等评测。
- quality runtime provider 负责只读 code review，产物写入 `artifacts/code-review-report.json`。
- quality orchestration 负责校验 handoff repo anchor、串联 gates、写入 `quality-report.json`，并在失败时生成 `quality-repair-handoff.json`。

当质量失败且仍有 repair budget 时，任务会通过审计流转回 `ready(programming)`，并复用既有 `failureRepair` metadata 与 `repair_goals`；当 handoff 缺失、repo anchor 不一致、关键 artifact 写入失败等基础前提不满足时，质量阶段 fail closed 到 `waiting`，等待人工或环境修复。
...
diting separates plugin concerns into three layers:
