...
```bash
export BACKEND_PORT=13000
export DATABASE_FILE=/var/lib/diting/sqlite/diting.sqlite
export DITING_WORKSPACE_ROOT=/var/lib/diting/workspaces
export DITING_WORKSPACE_REPO_CACHE_ROOT=/var/lib/diting/repos
export DITING_PLUGIN_EXECUTION_CODEX_BIN=codex
export DITING_PLUGIN_EXECUTION_CURSOR_BIN=agent
```
...
export DITING_PLUGIN_MEEGLE_WEBHOOK_SECRET=<secret>
```
...
2. 备份 .diting/repos 与必要 artifacts
3. 拉取新版本代码
4. 安装依赖并 build
5. 停旧进程
6. 执行 migration:run
...
- DATABASE_FILE
- .diting/repos
- 必要时 .diting/workspaces/artifacts
