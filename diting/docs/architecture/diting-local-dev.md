...
```bash
curl -X POST http://localhost:13000/api/tasks/<task-id>/validate
...
curl http://localhost:13000/api/traces/<trace-id>
```
...
```bash
export DITING_PLUGIN_MEEGLE_MODE=polling
export DITING_PLUGIN_MEEGLE_TASKS_FILE=$PWD/.diting/meegle/tasks.json
export DITING_PLUGIN_MEEGLE_RESULTS_FILE=$PWD/.diting/meegle/results.json
```
...
```bash
curl -X POST http://localhost:13000/api/debug/sync
```
...
```bash
export DITING_PLUGIN_MEEGLE_MODE=webhook
export DITING_PLUGIN_MEEGLE_WEBHOOK_SECRET=secret-1
```

发送 webhook：

```bash
curl -X POST http://localhost:13000/api/integrations/meegle/webhook \
  -H 'content-type: application/json' \
  -H 'x-diting-webhook-secret: secret-1' \
  -d '{
    "task": {
      "id": "MEEGLE-2",
      "title": "Webhook task",
...
- 正常空转时 idle agent 会由后台心跳保活；若全部 offline，优先确认后端是否刚重启、scheduler 是否运行、以及是否有异常关闭残留状态
- 看 /api/readiness 是否有必需插件不健康

### 任务进入 needs_human

- DITING_GOAL_ENABLE_NEEDS_HUMAN_LOOP=false 时：
...
- DITING_GOAL_ENABLE_NEEDS_HUMAN_LOOP=true 时：
