...
2. 任务卡在 queued：看 agent 是否都 offline / disabled，以及 scheduler 是否被跳过；正常空转时 idle agent 会由后台心跳保活，不应整体 offline。
3. 任务卡在 needs_human 或 waiting human_input：运行 diagnose:task，重点看失败 checks、risk level、repair goal 和 waitReason；产品 Agent 交互场景可先在 Web 控制台点击「检查回复」。
...
```bash
curl -X POST http://localhost:13000/api/tasks/<task-id>/recover \
...
产品 Agent 中途提问后，人工需要在 Meegle 最新评论前加 `【回复】` 标签，再在 Web 控制台点击「检查回复」。等价 API：

```bash
curl -X POST http://localhost:13000/api/tasks/<task-id>/sync-human-reply
```

该接口只读取父任务评论中的 `【回复】` 内容；质量失败修复方案仍使用「检查子任务方案」或 `POST /api/tasks/<task-id>/sync-human-repair-issue`，读取子任务描述里的 `【开发中】`。
...
- needs_human
  ...
  触发：
  POST /api/tasks/:id/needs-human 手工触发；或在 DITING_GOAL_ENABLE_NEEDS_HUMAN_LOOP=true 且插件支持评论闭环时，由 Goal Loop 的 high_risk / repeated_failure / no_effective_diff 自动触发。
- blocked
...
```bash
curl -X POST http://localhost:13000/api/agents/<agent-id>/recover
```
...
4. 如有需要，同时恢复 .diting/repos 和保留的 artifacts。
