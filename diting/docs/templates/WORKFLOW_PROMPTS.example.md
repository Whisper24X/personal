...
本文档是一个可选的 `WORKFLOW_PROMPTS.md` 模板骨架，用来说明：

- 如何声明默认执行流程
- 如何定义节点级 Prompt
- 如何约束“禁止追问、必须落盘、信息不足时做最小假设”

spec 压缩包必须包含 `openspec/`；`WORKFLOW_PROMPTS.md` 可省略。省略时，执行器会使用内置 Superpowers 默认 workflow。需要自定义节点顺序或提示词时，再按你的项目实际节点、技能和产物路径替换下面的占位内容，不要直接照搬节点名称或输出文件名。
...
> **多仓工作流**：如提供 `WORKFLOW_PROMPTS.md`，应放在 **任务工作区根目录**（由飞书 spec 物化），而非某个 Git 仓库内。CLI 默认 cwd 为 `{{workspacePath}}`；各仓路径见 `{{reposList}}`（`Repo1=/path/to/repos/foo` 格式）。
