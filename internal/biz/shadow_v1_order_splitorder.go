...
// SplitOrderToSubOrders 拆分订单为子订单
// 根据商品的所有商品类别中的 useTime 累加值，创建对应数量的子订单
// 金额平分到每个子订单，注意单位转换：父订单（元）-> 子订单（分）
func (s *ShadowV1OrderUseCase) SplitOrderToSubOrders(ctx context.Context, orderId string) error {
	// 1. 查询父订单
	order, err := s.orderRepo.FindOneByID(ctx, orderId)
	if err != nil {
		s.log.Errorf("拆单失败：查询父订单失败，orderId=%s, err=%v", orderId, err)
		return errorx.DataSQLErr.WithError(err).Err()
	}

	if order == nil || order.ID == "" {
		s.log.Errorf("拆单失败：订单不存在，orderId=%s", orderId)
		return errorx.DataSQLErr.Err()
	}

	// 记录父订单的手机号信息
	s.log.Infof("拆单：查询父订单信息，orderId=%s, Ph=%s", orderId, order.Ph)

	// 2. 检查是否已经拆过单
	existingSubOrders, err := s.subOrderRepo.FindMultiByParentOrderID(ctx, orderId)
	if err != nil {
		s.log.Errorf("拆单失败：查询子订单失败，orderId=%s, err=%v", orderId, err)
		return errorx.DataSQLErr.WithError(err).Err()
	}

	if len(existingSubOrders) > 0 {
		s.log.Infof("订单已经拆过单，跳过拆单，orderId=%s, 子订单数=%d", orderId, len(existingSubOrders))
		return nil
	}

	// 3. 查询商品信息
	goodInfo, err := s.goodRepo.FindOneByID(ctx, order.GoodID)
	if err != nil {
		s.log.Errorf("拆单失败：查询商品失败，orderId=%s, goodId=%s, err=%v", orderId, order.GoodID, err)
		return errorx.DataSQLErr.WithError(err).Err()
	}

	if goodInfo == nil || goodInfo.ID == "" {
		s.log.Errorf("拆单失败：商品不存在，orderId=%s, goodId=%s", orderId, order.GoodID)
		return errorx.DataSQLErr.Err()
	}
	...
	if err != nil {
		s.log.Errorf("拆单失败：解析商品内容失败，orderId=%s, goodId=%s, err=%v", orderId, order.GoodID, err)
	...
	if totalUseTimes <= 0 {
		s.log.Infof("订单不需要拆单，orderId=%s, totalUseTimes=%d", orderId, totalUseTimes)
		return nil
	}

	// 查询商品类型
	platformGoodIdToGoodTypeMap, err := s.platformGoodRepo.PlatformGoodIdToGoodType(ctx, []string{goodInfo.PlatformGoodID})
	if err != nil {
		s.log.Errorf("拆单失败：查询商品类型失败，orderId=%s, platformGoodId=%s, err=%v", orderId, goodInfo.PlatformGoodID, err)
		return errorx.DataSQLErr.WithError(err).Err()
	}
	goodType := platformGoodIdToGoodTypeMap[goodInfo.PlatformGoodID]

	// 4.1 查询父订单的预约记录，分析预约情况
	appointmentCounts := s.analyzeOrderAppointments(ctx, orderId)
	s.log.Infof("拆单：订单预约情况分析, orderId=%s, 已完成预约数=%d, 已预约数=%d, 总预约数=%d",
		orderId, appointmentCounts.completedCount, appointmentCounts.successCount, appointmentCounts.totalCount)

	// 5. 计算每个子订单的金额（单位转换：元 -> 分）
	// 父订单金额（元）* 100 = 总金额（分）
	// 使用四舍五入避免浮点数精度问题
	totalAmountInCents := int32(order.OrderPrice*100 + 0.5)
	// 优惠金额（分）
	...
	s.log.Infof("开始拆单，orderId=%s, totalUseTimes=%d, totalAmount=%d分, avgAmount=%d分, totalDiscount=%d分, avgDiscount=%d分",
	...
	for i := int32(0); i < totalUseTimes; i++ {
		...
		// 根据预约情况设置 ServiceStatus
		// 优先级：已完成 > 已预约 > 待预约
		var serviceStatus string
		if i < appointmentCounts.completedCount {
			// 前 completedCount 个子订单设置为已完成
			serviceStatus = string(constant.OrderStatusCompleted)
		} else if i < appointmentCounts.completedCount+appointmentCounts.successCount {
			// 接下来的 successCount 个子订单设置为已预约
			serviceStatus = string(constant.OrderStatusSuccess)
		} else {
			// 剩余的子订单设置为待预约
		...
		subOrder := &yanxue_model.SubOrder{
			...
			Status:                 string(constant.OrderStatusPending), // 支付成功
			...
			CertificateID:          order.CertificateID, // 继承父订单的券ID
		}

		// 记录子订单的手机号赋值情况
		if i == 0 {
			s.log.Infof("拆单：子订单手机号赋值，orderId=%s, 父订单Ph=%s, 子订单Ph=%s", orderId, order.Ph, subOrder.Ph)
		}
	...
	// 7. 批量创建子订单
	err = s.subOrderRepo.CreateBatchCache(ctx, subOrders, 100)
	if err != nil {
		s.log.Errorf("拆单失败：创建子订单失败，orderId=%s, err=%v", orderId, err)
		return errorx.DataSQLErr.WithError(err).Err()
	}

	s.log.Infof("订单拆分成功，orderId=%s, 拆分为 %d 个子订单", orderId, totalUseTimes)
	return nil
}

// appointmentCounts 预约统计信息
type appointmentCounts struct {
	completedCount int32 // 已完成的预约数量
	successCount   int32 // 已预约的数量
	totalCount     int32 // 总预约数量
}

// analyzeOrderAppointments 分析订单的预约情况
// 返回已预约和已完成的预约数量
func (s *ShadowV1OrderUseCase) analyzeOrderAppointments(ctx context.Context, orderId string) appointmentCounts {
	counts := appointmentCounts{}

	// 查询该订单的所有预约记录
	appointments, err := s.courseAppointmentRepo.FindMultiByOrderID(ctx, orderId)
	if err != nil {
		s.log.Errorf("analyzeOrderAppointments: 查询预约记录失败, orderId=%s, err=%v", orderId, err)
		// 查询失败时，返回空统计（所有子订单将设置为待预约）
		return counts
	}
	...
	for _, appointment := range appointments {
		...
		case string(constant.CourseAppointmentStatusCompleted): // 已完成
			counts.completedCount++
			counts.totalCount++
		case string(constant.CourseAppointmentStatusSuccess): // 已预约
			counts.successCount++
			counts.totalCount++
		}
		// 其他状态（如取消）不统计
