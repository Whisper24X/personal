...
const segmentTextIntoWords = text => {
  ...
  isWordBased =
    wordsCount > 1 &&
    getSegmentLengthMedian(segments) <= 13 &&
 ...
 */
const getCanvas = (component, selector) =>
  new Promise(resolve => {
    if (selector) {
      // 在 Taro 环境中，使用全局查询避免上下文问题
      let query
      if (typeof Taro !== "undefined" && Taro.createSelectorQuery) {
        query = Taro.createSelectorQuery()
        console.log("🔍 Canvas查询: 使用 Taro.createSelectorQuery 进行全局查询")
      } else if (typeof wx !== "undefined" && wx.createSelectorQuery) {
        query = wx.createSelectorQuery()
        console.log("🔍 Canvas查询: 使用 wx.createSelectorQuery 进行全局查询")
      } else {
        console.error("❌ 无法创建 Canvas SelectorQuery")
        resolve(null)
        return
      }
...
const drawImageRepeated = (
  ...
  if (repeatX) {
    if (stepX > -1 && x + (stepX + 1) * width < clipBox.right) {
      drawImageRepeated(ctx, clipBox, image, x, y, width, height, true, false, stepX + 1, stepY)
    }
...
class Canvas {
  ...
  setTransform() {
    ...
    // 变换前的中心点
    const { x, y } = inverseTransform(m11, m12, m21, m22, m41, m42, xTransformed, yTransformed)
    // 变换前的节点信息
  ...
  async drawVideo() {
    ...
    ctx.quadraticCurveTo(
      vertexes[0][0],
      vertexes[0][1],
      vertexes[0][0] + RADIUS[1],
      vertexes[0][1] + RADIUS[0]
    )
    ctx.lineTo(vertexes[1][0] - RADIUS[1], vertexes[1][1] - RADIUS[0])
    ctx.quadraticCurveTo(
      vertexes[1][0],
      vertexes[1][1],
      vertexes[1][0] - RADIUS[1],
      vertexes[1][1] + RADIUS[0]
    )
    ctx.lineTo(vertexes[2][0] + RADIUS[1], vertexes[2][1] - RADIUS[0])
    ctx.quadraticCurveTo(vertexes[2][0], vertexes[2][1], vertexes[2][0], vertexes[2][1] - RADIUS[2])
    ctx.closePath()
  ...
  drawText() {
    ...
    const isTextRightAlign =
      element["text-align"] === "right" || (isTextRTL && element["text-align"] === "start")
     ...
     * 向上取整避免行高过大，文字错位
     */
    const maxLines = Math.max(Math.ceil(Number(content.height / lineHeight).toFixed(1)), 1)
    // 消除行高计算偏差
    lineHeight = content.height / maxLines
    /** 单行内容，逐行显示 */
    let lineText = ""
    /** 内容基本单位拆分 */
    // 获取文本内容，优先使用dataset.text，如果没有则尝试使用其他方式
    let textContent = ""
    if (element.dataset.icon) {
      textContent = String.fromCharCode(parseInt(element.dataset.icon, 16))
    } else if (element.dataset.text) {
      textContent = element.dataset.text
    } else if (element.textContent) {
      // 如果dataset.text不存在，尝试使用textContent
      textContent = element.textContent
    } else {
      // 最后的兜底，使用空字符串
      console.warn("无法获取文本内容，element:", element)
      textContent = ""
    }
    ...
    const segments = segmentTextIntoWords(textContent)
    ...
    for (; lines < maxLines; lines += 1) {
      ...
      const lineLeft =
        (isTextRightAlign ? content.right : content.left) + // 首行缩进位置偏移
        (isTextRightAlign ? -1 : 1) * (lines === 0 ? textIndent : 0) + // 文字居中位置偏移
        (isTextRightAlign ? -1 : 1) * (isTextCentered ? lineWidth / 2 : 0)
      const lineTop = content.top + lines * lineHeight
      const lineTopOffset = (lineHeight - fontSize * FONT_SIZE_OFFSET) / 2
      ctx.fillText(lineText, lineLeft, lineTop + lineTopOffset, lineWidth)
    ...
    if (border.width > 0) {
      ...
      POSITIONS.map((key, index) => {
        ...
        if (border[key].style === "double") {
          ...
          const width =
            ["left", "right"].indexOf(key) > -1
              ? border[key].width
              : element.width + 2 * ctx.lineWidth
          const height =
            ["top", "bottom"].indexOf(key) > -1
              ? border[key].width
              : element.height + 2 * ctx.lineWidth
          if (key === "right") {
