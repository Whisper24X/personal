...
const getComputedRect = nodesRef => {
  let { left, right, bottom, top, width, height } = nodesRef
...
/** 节点固定样式名（可能与节点其他字段重名） */
Element.CONSTANT_COMPUTED_STYLE = ["width", "height", "left", "top", "right", "bottom"]
/** 节点通用样式名 */
...
/** 图片节点特殊属性名 */
Element.IMAGE_PROPERTIES = ["src", "mode"]
/** 图片节点特殊样式名 */
Element.IMAGE_COMPUTED_STYLE = []
/** 视频节点特殊属性名 */
Element.VIDEO_PROPERTIES = ["src", "object-fit", "poster"]
/** 视频节点特殊样式名 */
Element.VIDEO_COMPUTED_STYLE = []
/** 视频节点特殊属性名 */
Element.CANVAS_PROPERTIES = ["type", "canvas-id"]
/** 画布节点特殊属性名 */
 ...
 */
Element.getNodesRef = (selector, fields, page, component) =>
  new Promise(resolve => {
    // 在 Taro 环境中，使用全局查询，不指定上下文
    let query
    if (typeof Taro !== "undefined" && Taro.createSelectorQuery) {
      query = Taro.createSelectorQuery()
      // 不使用 query.in() 方法，直接进行全局查询
      console.log("🔍 使用 Taro.createSelectorQuery 进行全局查询")
    } else if (typeof wx !== "undefined" && wx.createSelectorQuery) {
      query = wx.createSelectorQuery()
      console.log("🔍 使用 wx.createSelectorQuery 进行全局查询")
    } else {
      console.error("❌ 无法创建 SelectorQuery")
      resolve([])
      return
    }
