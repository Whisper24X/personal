...
2. **读取基准文档**：
   - 读取 `../skills/project-management/story-points-baseline.md`（评估标准）
   - 理解故事点定义和评估维度

3. **分析任务复杂度**（参考评估基准）：
   - **接口数量**：需要开发的 API 接口数
   - **UI页面数**：需要开发的页面/组件数
   - **业务逻辑复杂度**：业务规则、状态机、权限控制等
   - **数据表数量**：需要创建或修改的数据库表数
   - **状态管理复杂度**：前端状态管理、数据流复杂度

4. **对比评估基准**：
   - 以 **3 个故事点的应用更新功能** 作为基准点
   - 估算每个任务的相对复杂度
...
| 故事点 | 复杂度       | 示例                                 |
| ------ | ------------ | ------------------------------------ |
| 1      | 非常简单     | 单个简单 API、单个简单页面           |
| 2      | 简单         | 2-3个简单 API、简单 CRUD 页面        |
| 3      | 中等（基准） | 应用更新功能（1个表+3个API+1个页面） |
| 5      | 复杂         | 多表关联、复杂业务逻辑、复杂 UI      |
| 8      | 非常复杂     | 核心功能模块、多模块联动             |
