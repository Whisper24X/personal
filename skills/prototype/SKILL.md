...
<html lang="zh-CN">
  ...
  <body>
    ...
    <script>
      ...
      createApp({
        setup() {
          // ✅ 所有响应式变量必须给初始值，禁止使用 null/undefined
          const title = ref('原型标题');
          const list = ref([]); // 列表初始为空数组，非 null
          const detail = ref({}); // 对象初始为 {}，非 null
          const loading = ref(false);

          // ✅ 访问嵌套属性用可选链，防止 TypeError
          const displayName = computed(() => detail.value?.name ?? '未命名');

          return { title, list, detail, loading, displayName };
        },
      })
        .use(ElementPlus)
        .mount('#app');
    ...
    <script>
      ...
      createApp({
        setup() {
          ...
          const list = ref([]); // 列表初始为空数组，非 null
          const selected = ref(null); // 可为 null，但访问属性时必须用可选链

          // ✅ 访问可能为 null 的对象属性，使用可选链
          const selectedTitle = computed(() => selected.value?.title ?? '');

          return { title, list, selected, selectedTitle };
        },
      }).mount('#app');
    </script>
  </body>
