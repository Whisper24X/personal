...
```go
// BatchCreate 商品表-批量创建
func (p *ShadowV1ProductUseCase) BatchCreate(ctx context.Context, req *pb.BatchCreateReq) error {
    return p.commonRepo.Transaction(ctx, func(tx *studyspace_service_dao.Query) error {
        for _, item := range req.Items {
            product := &studyspace_service_model.Product{Name: item.Name}
