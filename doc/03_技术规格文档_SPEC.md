...
- ✅ Engineer（工程师）- 代码实现
- ✅ QAEngineer（QA工程师）- 测试设计工作流（3步）
- ✅ AutomationEngineer（自动化工程师）- 自动化测试工作流（4步）
...
**功能描述**: 通过RAG技术为角色提供上下文知识支持，确保完整迭代产出

**输入**:
- 项目配置（applicationId, version）
...
**失败场景**:
- 知识库不存在 → 使用默认空知识库继续执行
- 检索失败 → 记录警告，继续执行（不使用知识库）
- 代码仓库访问失败 → 提示用户检查权限

**核心特性**:
- **项目级知识库**: 每个应用下的项目关联专属知识库
- **代码仓库关联**: 支持Git仓库或本地代码仓库
- **智能检索**: 基于向量数据库的语义检索
- **迭代增强**: 每次迭代完成后自动更新知识库

#### F8. 工作流编排系统

**功能描述**: 支持多角色直接串联的自定义工作流，灵活配置执行顺序和输入输出映射
...
**核心特性**:
- **独立执行**: 不依赖其他角色或工作流
- **断点调试**: 支持在特定Action处设置断点
- **性能监控**: 监控执行时间、Token使用等
- **单元测试**: 支持角色单元测试框架
...
```python
{
    ...
    "knowledge_base": KnowledgeBase # 知识库（可选）
}
```
**生命周期**: 应用启动到结束  
**持久化**: 部分（配置持久化）
...
```python
{
    ...
    "version": str,                # 版本号
    "documents": list[Document],   # 文档知识库
    "code_repositories": list[CodeRepo], # 代码仓库
    "apis": list[APIDoc],          # API文档库
...
WorkflowStep:
{
    "id": str,                      # 步骤ID
    "role": str,                    # 角色名称
    "actions": list[str],           # Action列表
    "input": {                      # 输入配置
        "source": str | list[str],  # 输入来源
        "mapping": dict             # 输入映射
    },
    "output": {                     # 输出配置
        "target": str | list[str],  # 输出目标
        "mapping": dict             # 输出映射
    },
...
- ✅ 知识库系统使用RAG技术（向量数据库 + 语义检索）
- ✅ 工作流编排支持多角色串联和输入输出映射
- ✅ 角色独立调试使用专门的调试接口和测试框架
