...
如 sandbox 环境可用（容器已启动且后端服务正常），对本次新增或修改的后端接口执行基本调用验证。

**执行方式**：

1. **确定接口清单**：从 applyResult.md 的文件列表中提取涉及的后端接口（如新增的 proto HTTP 注解、service 文件等），汇总出所有新增/修改的接口路径
2. **逐接口调用验证**：使用 curl 对每个接口发送最简请求（空参数或仅必填参数），检查响应
3. **判定标准**：
   - 通过：返回正常业务响应（含空列表、空数据等正常结果）
   - 失败：返回 500 错误、框架级异常（如字段不匹配、表不存在、权限配置错误等）
   - 忽略：返回 401/403（鉴权问题，非代码逻辑错误）

**注意事项**：
- 若 sandbox 未启动或后端服务不可用，跳过此步骤并在输出中注明
- 发现的接口错误视为"高严重程度"问题，必须在第 5 步中修复
- 修复代码后等待服务热加载完成，再重新调用接口确认问题已解决
...
3. **实现修复**：
   ...
   - 空函数体 → 补全函数实现
   - 占位符 → 替换为实际代码
   - 接口运行时错误 → 根据错误信息定位并修复代码
4. **验证修复**：确保修复后代码完整且可编译；接口错误修复后需等待热加载完成并重新调用验证
...
- AI 检查：发现 2 个问题
- 工具检查：发现 1 个问题
- 接口检查：发现 1 个问题（或：跳过-sandbox 不可用）
- 发现问题：3
- 修复问题：3
