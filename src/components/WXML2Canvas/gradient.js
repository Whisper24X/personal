import { TRI2RAD_RATIO, SIDE2CORNER_RATIO, IS_WINDOWS } from "./constants"
...
export const createLinearGradient = (ctx, element) => {
  ...
  /** 渐变角度描述内容 */
  const [angleContent] =
    gradientContent.match(/(-?\d+(\.\d+)?(turn|deg|grad|rad))|(to( (left|top|bottom|right))+)/) ??
    []
  ...
  if (/-?\d+(\.\d+)?(turn|deg|grad|rad)/.test(angleContent)) {
    ...
    let angleRatio = 1
    if (/turn/.test(angleContent)) {
      // 圈数
      roundAngle = 1
      angleRatio = 360 / roundAngle
    } else if (/grad/.test(angleContent)) {
      // 百分度数
      roundAngle = 400
      angleRatio = 360 / roundAngle
    } else if (/rad/.test(angleContent)) {
      // 弧度数
  ...
  } else if (angleContent === "to left") {
    // 从右往左
    gradientAngle = -90
  } else if (angleContent === "to right") {
    // 从左往右
    gradientAngle = 90
  } else if (angleContent === "to top") {
    // 从下往上
    gradientAngle = 0
  } else if (/( (left|top|bottom|right)){2}/.test(angleContent)) {
    // 斜上 或 斜下，对角方向
    gradientAngle =
  ...
  // 渐变角度 与 水平线 的夹角（锐角）
  const gradientAngleDiffHorizon =
    90 - (isObtuse ? 180 - Math.abs(gradientAngle) : Math.abs(gradientAngle))
  /** 渐变角度 与 矩形斜边 的夹角（锐角） */
  const gradientAngleDiffDiagonal =
  ...
  /** 渐变颜色描述内容 */
  const colorsContent =
    gradientContent.match(
      /((rgba?\(((, )?\d+(\.\d+)?)+\)|red)( -?\d+(\.\d+)?(px|%))*)|(-?\d+(\.\d+)?(px|%))/g
    ) ?? []
  /** 渐变色标位置集合 */
  const colorStops = colorsContent.map((item, index) => {
    ...
    /** 渐变色标位置 */
    const stops = (item.match(/-?\d+(\.\d+)?(px|%)/g) ?? []).map(
      stopItem => parseFloat(stopItem) * (/%/.test(stopItem) ? gradientDiagonal / 100 : 1)
    )
    /** 渐变色标信息 */
    const colorStop = { stops }
    if (color) {
      Object.assign(colorStop, { color })
    }
  ...
  startingPoint.push(
    (isClockwise ? content.left : content.right) +
      (isObtuse ? 1 : -1) * (isClockwise ? 1 : -1) * startingPointX -
      startingPrefixX,
...
export const createRadialGradient = (ctx, element) => {
  ...
  /** 渐变的位置 */
  const [position] =
    gradientContent.match(/at( (left|top|bottom|right|center|(-?\d+(\.\d+)?(px|%))))+/) ?? []
  /** 渐变的形状 */
  let [endingShape] = gradientContent.match(/ellipse|circle/) ?? ["ellipse"]
  /** 渐变结束形状的大小描述 */
  const [sizeExtent] = gradientContent.match(
  ...
  const colorStops = colorsContent.map(item => {
    ...
    /** 渐变色标位置 */
    const stops = (item.match(/-?\d+(\.\d+)?(px|%)/g) ?? []).map(
      stopItem => parseFloat(stopItem) * (/%/.test(stopItem) ? radius / 100 : 1)
    )
    /** 渐变色标信息 */
    const colorStop = { stops }
    if (color) {
  ...
  for (let index = 0; index < colorStops.length; index++) {
    ...
    for (; stopIndex < item.stops.length; stopIndex++) {
      const stopItem = item.stops[stopIndex]
      /** 色标位置偏移值 */
      const stopOffset = +Number(stopItem / realRadius).toFixed(4)
      if (stopOffset > 1 || stopOffset < 0) break
      gradient.addColorStop(stopOffset, item.color)
    }
  }
...
export const createConicGradient = (ctx, element) => {
  ...
  const colorsContent =
    gradientContent.match(
      /((rgba?\(((, )?\d+(\.\d+)?)+\)|red)( -?\d+(\.\d+)?(turn|deg|grad|rad|%))*)|(-?\d+(\.\d+)?(turn|deg|grad|rad|%))/g
    ) ?? []
  /** 渐变起始角度校准 */
  let angleOffset = 0
  /** 渐变色标位置集合 */
  const colorStops = colorsContent.map(item => {
    ...
    const [color] = item.match(/rgba?\(((, )?\d+(\.\d+)?)+\)|red/) ?? []
    /** 渐变色标位置 */
    const stops = (item.match(/-?\d+(\.\d+)?(turn|deg|grad|rad|%)/g) ?? []).map(stopItem => {
      let stopAngle
  ...
  const gradient = ctx.createConicGradient(
    ...
    content.left + positionX,
    content.top + positionY
  )
  ...
  for (let index = 0; index < colorStops.length; index++) {
    ...
    for (; stopIndex < item.stops.length; stopIndex++) {
      const stopItem = item.stops[stopIndex]
      /** 色标位置偏移值 */
      const stopOffset = +Number(stopItem / (2 * Math.PI)).toFixed(4)
      if (stopOffset > 1 || stopOffset < 0) break
      gradient.addColorStop(stopOffset, item.color)
    }
  }
