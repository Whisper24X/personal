<template>
  <view v-if="show" class="date-picker-popup" @tap="handleClose">
    <view class="date-picker-content" @tap.stop>
      <view
        class="date-picker-header"
        :style="{
          backgroundImage: `${titleBg ? `url(${titleBg})` : `url('https://fp.yangcong345.com/middle/1.0.0/calendar-bg__w.png')`}`
        }"
        @tap="handleClose"
      >
        <!-- <text class="date-picker-title">选择日期</text>
        <view class="date-picker-close" @tap="handleClose">×</view> -->
      </view>
      <view class="calendar">
        ...
        <view class="days">
          ...
          >
            <text>{{ day.day }}</text>
            <text v-if="day.isToday && !day.stockRemain" class="day-label">今天</text>
            <text v-if="day.stockRemain && day.stockRemain > 0" class="stock-info">
              剩余{{ day.stockRemain }}人
            </text>
          </view>
        </view>
      </view>
...
.date-picker-popup {
  ...
  .date-picker-header {
    height: 72px;
    background: url("../assets/calendar-bg.png");
  ...
  .date-picker-footer {
    padding: 0 32px;
    background-color: #fff;
    padding-bottom: calc(constant(safe-area-inset-bottom) + 32px); /* iOS 11.0 */
    padding-bottom: calc(env(safe-area-inset-bottom) + 32px); /* iOS 11.2+ */
  }
}
