<template>
  ...
  >
    ...
    <view v-else class="refunded-page">
      <view class="page-content">
        <!-- 退款成功状态 -->
        <OrderStatusHeader title="已退款成功" />

        <!-- 退款信息 -->
        <InfoCard title="退款信息">
          <InfoRow label="退款金额">
            <view class="refund-amount-row">
              <text class="refund-amount-text">(已原路退回)</text>
              <text class="refund-amount">¥{{ formatAmount(refundAmount) }}</text>
            </view>
          </InfoRow>
          <InfoRow label="退款时间">{{ formatDateTime(refundTime) }}</InfoRow>
        </InfoCard>
...
const refundAmount = computed(() => {
  return centsToYuan(orderData.value?.orderInfo?.refundAmount || 0)
})

// 退款时间（待接口提供，暂时使用支付时间）
const refundTime = computed(() => {
  return orderData.value?.orderInfo?.refundTime
})
