...
部分环境与智齿文档中，`items` 单条还包含 `deal_agentid`、`deal_groupid`（或 `deal_agent_groupid` 与列表文档对齐）、`update_agent_name` 等；以实际返回为准，与 **§4.4** 对照。
...
## 4. get_ticket_by_id（查询工单详情）

**官方文档**：[工单 API — 查询工单详情页](https://developer.zhichi.com/pages/950d89/#%E2%97%8F-%E6%9F%A5%E8%AF%A2%E5%B7%A5%E5%8D%95%E8%AF%A6%E6%83%85%E9%A1%B5)。

**本仓库约定 Host**（与 SKILL 一致）：

`https://www.sobot.com/api/ws/{ws_id}/ticket/get_ticket_by_id`

示例：`https://www.sobot.com/api/ws/5/ticket/get_ticket_by_id`
...
| token | String | 是 | 开放平台 Token |
| language | String | 否 | 语言等 |
| timezoneid | String | 否 | 时区，如 `Asia/Shanghai` |

- **Query 参数**

| 参数 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| ticketid | String | 与 ticket_code 二选一 | 工单 ID |
| ticket_code | String | 与 ticketid 二选一 | 工单编号 |

**说明**：官方文档称 **同时**传入 `ticketid` 与 `ticket_code` 时，以 **工单 ID** 为准；至少 **一个** 必填。

### 4.2 响应要点

- 顶层含 `ret_code`、`ret_msg`；成功时 `item` 为工单详情对象。
- **`item.update_log_list`**：操作记录（工单操作历史）。
- **`item.deal_list`**：工单回复列表（客服与客户回复）。
- 其它字段与列表接口中单条工单字段类似（见 §3.3），详情以智齿文档为准；**受理客服 / 更新客服与表格列映射** 见 §4.4。
...
curl -H 'token: <YOUR_TOKEN>' \
  -H 'timezoneid: Asia/Shanghai' \
  'https://www.sobot.com/api/ws/5/ticket/get_ticket_by_id?ticketid=<TICKET_ID>'
```
...
curl -H 'token: <YOUR_TOKEN>' \
  -H 'timezoneid: Asia/Shanghai' \
  'https://www.sobot.com/api/ws/5/ticket/get_ticket_by_id?ticket_code=<TICKET_CODE>'
```

### 4.4 受理客服、更新客服与表格列（智齿文档要点）

与 [SKILL.md](SKILL.md)「智能硬件与表格写入规则」配套使用。

**列表 `items` 中单条**（在 §3.3 基础上，智齿「根据更新时间查询工单列表」文档常见字段）

| 参数 | 说明 |
| --- | --- |
| deal_agentid | 受理坐席 ID |
| deal_agent_name | 受理坐席名称 |
| deal_groupid | 受理技能组 ID（与 Query 中 `deal_agent_groupid` 同义对接时以环境为准） |
| deal_group_name | 受理技能组名称 |
| update_agent_name | 操作坐席名称（与后台「更新人」展示一致时可作更新客服） |

**详情 `item`**

| 参数 | 说明 |
| --- | --- |
| deal_agentid | 工单受理坐席 ID（流程工单可能多个，逗号分隔） |
| deal_agent_name | 工单受理坐席名称 |
| deal_groupid | 工单受理技能组 ID |
| deal_group_name | 工单受理技能组名称 |
| update_agent_name | 工单更新人名称 |

**`item.update_log_list[]` 单条操作记录**

| 参数 | 说明 |
| --- | --- |
| update_agent_name | 操作人名称 |
| update_agent_id | 操作人 ID；`start_type` 为坐席/客户时有值（坐席时为客服 ID） |
| start_type | 操作人类型（坐席/客户/触发器等，见智齿文档） |
| update_datetime | 操作记录时间 |

**写入表格时「更新客服」四列**（业务列名）：受理客服组 ID、受理客服组、受理客服 ID、受理客服（指 **更新操作人** 在 mapping 中对应的组与坐席）。若 `item` 未直接返回更新人的组字段，用 `update_agent_name` 或 `update_agent_id`（来自详情或 `update_log_list` 坐席记录）反查 [受理id-mapping.md](受理id-mapping.md)；以实际返回为准。

---

## 5. 受理客服 / 受理客服组 ID 对照（本地表）

- **Markdown 全表（推荐代理检索）**：[受理id-mapping.md](受理id-mapping.md)
- **Excel 源文件**：[智齿受理id.xlsx](智齿受理id.xlsx)

与 Query 的对应关系：**受理客服ID** → `deal_agentid`；**受理客服组ID** → `deal_agent_groupid`。接口通常**不支持**仅按 `deal_agent_name` 过滤时，先从本表解析 ID 再请求。

---

## 6. 常见错误码（与 Token 相关，摘自智齿文档常见项）
