 ...
 */
async function loadDocumentsFromWorkspace(applicationId: string, projectId: string): Promise<{ prd: string; design: string; taskBreakdown: string }> {
  if (!applicationId) {
    throw new Error(
      'applicationId is required for loadDocumentsFromWorkspace. Cannot use "default" to prevent file conflicts between different applications.'
    );
  }
  if (!projectId) {
    throw new Error(
      'projectId is required for loadDocumentsFromWorkspace. Cannot use "default" to prevent file conflicts between different projects.'
    );
  }
  const result = {
    prd: '',
  ...
  try {
    ...
    const foundDocs = [result.prd && 'PRD', result.design && 'DESIGN', result.taskBreakdown && 'TASKS'].filter(Boolean);
...
export async function testWriteCode(req: Request, res: Response) {
  try {
    ...
    // Add ExecuteProjectManagement message if provided (replaces deprecated individual OpenSpec actions)
    if (taskBreakdown) {
      const breakdownMessage = new Message({
        content: taskBreakdown,
        role: 'ProjectManager',
        causeBy: 'ExecuteProjectManagement',
...
export async function testExecuteSubtask(req: Request, res: Response) {
  try {
    ...
    if (!taskBreakdown) {
      return res.status(400).json({
        error:
          'taskBreakdown is required for ExecuteSubtask. Please provide in request body or ensure TASK_BREAKDOWN.md exists in workspace/{applicationId}/{projectId}/ainative-workspace/docs/tasks/',
      });
    }
    ...
    // Add ExecuteProjectManagement message (replaces deprecated individual OpenSpec actions)
    const breakdownMessage = new Message({
      content: taskBreakdown,
      role: 'ProjectManager',
      causeBy: 'ExecuteProjectManagement',
...
export async function testDeploy(req: Request, res: Response) {
  try {
    const { design: providedDesign, workspaceOptions, llmConfig } = req.body as EngineerTestRequest;
...
export async function testImproveCode(req: Request, res: Response) {
  try {
    const { workspaceOptions, llmConfig } = req.body as EngineerTestRequest;
