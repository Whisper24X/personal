...
4. **生成部署文档**
   ...
   - 对于未启动或启动失败的服务，明确标注状态（如：未启动、目录不存在、启动失败等）
...
2. **验证服务可访问性**
   ...
   - 只对应该运行的服务进行访问测试（排除明确标注为"未启动"、"启动失败"或"目录不存在"的服务）
   - 使用 curl 或其他方式验证，检查返回状态码：
     * 2xx 状态码 → 成功
     * 3xx 重定向 → 需要说明（一般视为成功）
     * 4xx、5xx → 失败
     * Connection refused/timeout → 失败
   - 每个地址都需要实际访问验证
   - **关键原则：deploy.md 中所有应该运行的服务都必须可正常访问**

3. **返回检查结果**
   以 JSON 格式返回，包含：
   - result 字段：
     * "已完成" - 仅当所有应该运行的服务都可正常访问（返回 2xx 状态码）
     * "未完成" - 任何一个应该运行的服务无法访问或返回错误状态码
     * "未找到" - deploy.md 文件不存在
   - reason 字段：说明具体原因和检查结果
   - details 字段：包含每个服务的访问测试结果（状态码、响应信息等）
   
   **判断标准：**
   - 所有应该运行的服务必须返回 2xx 状态码
   - 标记为"未启动"、"启动失败"或"目录不存在"的服务不参与检查
   - 只要有一个应该运行的服务失败，整体结果就是"未完成"
...
{
  "result": "已完成",
  "reason": "deploy.md 已创建，所有应该运行的服务均可正常访问",
  "details": {
    "统一入口": "✅ 200 OK",
    "后端API": "✅ 200 OK",
    "管理后台": "✅ 200 OK"
...
{
  "result": "未完成",
  "reason": "部分服务无法访问或返回错误状态码",
  "details": {
    "统一入口": "✅ 200 OK",
    "后端API": "❌ 502 Bad Gateway",
    "管理后台": "✅ 200 OK"
  }
}
\`\`\`
