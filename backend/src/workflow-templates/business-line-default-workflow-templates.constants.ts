...
export type BusinessLineDefaultWorkflowTemplateDefinition = {
  name: string;
...
/**
 * Seeded for each new business line (skeleton nodes: no Agent CLI until configured in BLM).
 */
export const BUSINESS_LINE_DEFAULT_WORKFLOW_TEMPLATES: BusinessLineDefaultWorkflowTemplateDefinition[] =
  [
    {
      name: '默认工作流',
      ...
      nodes: [
        {
          ...
          input: {
            prompt: `使用 \`brainstorming\` 技能，根据以下需求生成需求澄清文档：
{{taskPrompt}}
...
如果需求或原型信息不足，只允许做最小必要假设；所有假设必须单独列在“假设与待确认项”中，不得混入正式结论。`,
        ...
        },
        {
          ...
          requiresArtifact: false,
          input: {
            prompt: `使用 \`code-doc\` 技能，扫描当前工作空间未提交改动，按技能内模板生成接口变更清单与功能变更测试清单。
