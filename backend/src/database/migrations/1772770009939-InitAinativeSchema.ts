...
export class InitAinativeSchema1772770009939 implements MigrationInterface {
  name = 'InitAinativeSchema1772770009939';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "tasks" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "businessLineId" uuid NOT NULL, "projectId" uuid NOT NULL, "workflowTemplateId" uuid, "mode" "public"."task_mode_enum" NOT NULL, "title" character varying(160) NOT NULL, "prompt" text, "status" "public"."task_status_enum" NOT NULL DEFAULT 'todo', "gitBranch" character varying(120), "gitBaseBranch" character varying(120), "gitWorktree" character varying(500), "clientInputSnapshot" jsonb, "cliToolId" character varying(64), "agentToolConfigId" uuid, "createdBy" uuid, "startedAt" TIMESTAMP, "finishedAt" TIMESTAMP, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "deletedAt" TIMESTAMP, CONSTRAINT "PK_8d12ff38fcc62aaba2cab748772" PRIMARY KEY ("id")); COMMENT ON COLUMN "tasks"."id" IS '主键（UUID）'; COMMENT ON COLUMN "tasks"."businessLineId" IS '所属业务线ID'; COMMENT ON COLUMN "tasks"."projectId" IS '关联项目ID'; COMMENT ON COLUMN "tasks"."workflowTemplateId" IS '关联工作流模板ID'; COMMENT ON COLUMN "tasks"."mode" IS '任务模式'; COMMENT ON COLUMN "tasks"."title" IS '任务标题'; COMMENT ON COLUMN "tasks"."prompt" IS '任务提示词'; COMMENT ON COLUMN "tasks"."status" IS '任务状态'; COMMENT ON COLUMN "tasks"."gitBranch" IS '任务Git分支名称'; COMMENT ON COLUMN "tasks"."gitBaseBranch" IS '任务工作树基线分支'; COMMENT ON COLUMN "tasks"."gitWorktree" IS '任务Git工作树文件系统路径'; COMMENT ON COLUMN "tasks"."clientInputSnapshot" IS '客户端输入快照JSON'; COMMENT ON COLUMN "tasks"."cliToolId" IS 'CLI工具标识'; COMMENT ON COLUMN "tasks"."agentToolConfigId" IS '关联工具配置ID'; COMMENT ON COLUMN "tasks"."createdBy" IS '创建者用户ID'; COMMENT ON COLUMN "tasks"."startedAt" IS '任务执行开始时间'; COMMENT ON COLUMN "tasks"."finishedAt" IS '任务执行结束时间'; COMMENT ON COLUMN "tasks"."createdAt" IS '创建时间'; COMMENT ON COLUMN "tasks"."updatedAt" IS '更新时间'; COMMENT ON COLUMN "tasks"."deletedAt" IS '软删除时间'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE "task_logs" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "taskId" uuid NOT NULL, "taskNodeId" uuid, "level" "public"."task_log_level_enum" NOT NULL DEFAULT 'info', "message" text NOT NULL, "payload" jsonb, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_9754457a29b4ffbb772e8a3039c" PRIMARY KEY ("id")); COMMENT ON COLUMN "task_logs"."id" IS '主键（UUID）'; COMMENT ON COLUMN "task_logs"."taskId" IS '关联任务ID'; COMMENT ON COLUMN "task_logs"."taskNodeId" IS '关联任务节点ID'; COMMENT ON COLUMN "task_logs"."level" IS '日志级别'; COMMENT ON COLUMN "task_logs"."message" IS '日志消息内容'; COMMENT ON COLUMN "task_logs"."payload" IS '结构化日志载荷JSON'; COMMENT ON COLUMN "task_logs"."createdAt" IS '创建时间'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE "task_nodes" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "taskId" uuid NOT NULL, "nodeOrder" integer NOT NULL, "name" character varying(160) NOT NULL, "nodeType" "public"."task_node_type_enum" NOT NULL, "input" jsonb, "output" jsonb, "requiresApproval" boolean NOT NULL DEFAULT false, "status" "public"."task_node_status_enum" NOT NULL DEFAULT 'todo', "attempt" integer NOT NULL DEFAULT '0', "errorCode" character varying(120), "errorMessage" text, "startedAt" TIMESTAMP, "finishedAt" TIMESTAMP, "workerId" character varying(120), "leaseUntil" TIMESTAMP, "heartbeatAt" TIMESTAMP, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "UQ_task_nodes_task_node_order" UNIQUE ("taskId", "nodeOrder"), CONSTRAINT "PK_389cdd6e0626609626f572c2877" PRIMARY KEY ("id")); COMMENT ON COLUMN "task_nodes"."id" IS '主键（UUID）'; COMMENT ON COLUMN "task_nodes"."taskId" IS '关联任务ID'; COMMENT ON COLUMN "task_nodes"."nodeOrder" IS '节点在任务中的顺序'; COMMENT ON COLUMN "task_nodes"."name" IS '节点名称'; COMMENT ON COLUMN "task_nodes"."nodeType" IS '节点类型'; COMMENT ON COLUMN "task_nodes"."input" IS '节点输入JSON'; COMMENT ON COLUMN "task_nodes"."output" IS '节点输出JSON'; COMMENT ON COLUMN "task_nodes"."requiresApproval" IS '节点是否需要人工审批'; COMMENT ON COLUMN "task_nodes"."status" IS '节点状态'; COMMENT ON COLUMN "task_nodes"."attempt" IS '重试次数'; COMMENT ON COLUMN "task_nodes"."errorCode" IS '错误码'; COMMENT ON COLUMN "task_nodes"."errorMessage" IS '错误详情'; COMMENT ON COLUMN "task_nodes"."startedAt" IS '节点执行开始时间'; COMMENT ON COLUMN "task_nodes"."finishedAt" IS '节点执行结束时间'; COMMENT ON COLUMN "task_nodes"."workerId" IS '持有租约的工作进程ID'; COMMENT ON COLUMN "task_nodes"."leaseUntil" IS '租约过期时间'; COMMENT ON COLUMN "task_nodes"."heartbeatAt" IS '最近工作进程心跳时间'; COMMENT ON COLUMN "task_nodes"."createdAt" IS '创建时间'; COMMENT ON COLUMN "task_nodes"."updatedAt" IS '更新时间'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE "task_artifacts" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "taskId" uuid NOT NULL, "taskNodeId" uuid, "artifactType" "public"."task_artifact_type_enum" NOT NULL, "name" character varying(200) NOT NULL, "downloadUrl" text, "content" text, "metadata" jsonb, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_c2ec4d80514c8ccdcf6adf8679e" PRIMARY KEY ("id")); COMMENT ON COLUMN "task_artifacts"."id" IS '主键（UUID）'; COMMENT ON COLUMN "task_artifacts"."taskId" IS '关联任务ID'; COMMENT ON COLUMN "task_artifacts"."taskNodeId" IS '关联任务节点ID'; COMMENT ON COLUMN "task_artifacts"."artifactType" IS '工件类型'; COMMENT ON COLUMN "task_artifacts"."name" IS '工件名称'; COMMENT ON COLUMN "task_artifacts"."downloadUrl" IS '工件下载地址'; COMMENT ON COLUMN "task_artifacts"."content" IS '内联工件内容'; COMMENT ON COLUMN "task_artifacts"."metadata" IS '工件元数据JSON'; COMMENT ON COLUMN "task_artifacts"."createdAt" IS '创建时间'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `COMMENT ON TABLE "task_artifacts" IS '任务执行产生的工件'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."business_line_invitation_role_enum" AS ENUM('owner', 'admin', 'member')`,
    );
    await queryRunner.query(
      `CREATE TABLE "business_line_invitations" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "businessLineId" uuid NOT NULL, "token" character varying(128) NOT NULL, "role" "public"."business_line_invitation_role_enum" NOT NULL, "projectRoles" jsonb NOT NULL DEFAULT '{}'::jsonb, "createdBy" uuid NOT NULL, "expiresAt" TIMESTAMP NOT NULL, "revokedAt" TIMESTAMP, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_2cd94765894fc2895c60732725b" PRIMARY KEY ("id")); COMMENT ON COLUMN "business_line_invitations"."id" IS '主键（UUID）'; COMMENT ON COLUMN "business_line_invitations"."businessLineId" IS '关联业务线ID'; COMMENT ON COLUMN "business_line_invitations"."token" IS '邀请令牌'; COMMENT ON COLUMN "business_line_invitations"."role" IS '邀请成员角色'; COMMENT ON COLUMN "business_line_invitations"."projectRoles" IS '按项目分配的成员角色JSON'; COMMENT ON COLUMN "business_line_invitations"."createdBy" IS '邀请创建者用户ID'; COMMENT ON COLUMN "business_line_invitations"."expiresAt" IS '邀请过期时间'; COMMENT ON COLUMN "business_line_invitations"."revokedAt" IS '邀请撤销时间'; COMMENT ON COLUMN "business_line_invitations"."createdAt" IS '创建时间'; COMMENT ON COLUMN "business_line_invitations"."updatedAt" IS '更新时间'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE "agent_tool_configs" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "businessLineId" uuid NOT NULL, "toolId" character varying(64) NOT NULL, "name" character varying(120) NOT NULL, "description" character varying(255), "configJson" text NOT NULL, "isDefault" boolean NOT NULL DEFAULT false, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_b0c1292b22450a82edbe7d828d8" PRIMARY KEY ("id")); COMMENT ON COLUMN "agent_tool_configs"."id" IS '主键（UUID）'; COMMENT ON COLUMN "agent_tool_configs"."businessLineId" IS '关联业务线ID'; COMMENT ON COLUMN "agent_tool_configs"."toolId" IS '工具标识'; COMMENT ON COLUMN "agent_tool_configs"."name" IS '配置名称'; COMMENT ON COLUMN "agent_tool_configs"."description" IS '配置描述'; COMMENT ON COLUMN "agent_tool_configs"."configJson" IS '工具配置JSON'; COMMENT ON COLUMN "agent_tool_configs"."isDefault" IS '是否默认配置'; COMMENT ON COLUMN "agent_tool_configs"."createdAt" IS '创建时间'; COMMENT ON COLUMN "agent_tool_configs"."updatedAt" IS '更新时间'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `COMMENT ON TABLE "agent_tool_configs" IS '业务线工具配置'`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "agent_tool_configs" ADD CONSTRAINT "FK_d97b6748898e84e2b019be17052" FOREIGN KEY ("businessLineId") REFERENCES "business_lines"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "agent_tool_configs" DROP CONSTRAINT "FK_d97b6748898e84e2b019be17052"`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(`COMMENT ON TABLE "agent_tool_configs" IS NULL`);
    await queryRunner.query(
    ...
    await queryRunner.query(
      `DROP TYPE "public"."business_line_invitation_role_enum"`,
    );
