...
| MCP / project tool config | **Host-side MCP processes + URL/env mapping into the task container** (preferred) | Task containers are isolated: **stdio MCP servers should run on the Docker host** (or another routable address), bind ports there, and expose **HTTP/SSE URLs** to the Agent CLI via `Project.configJson.containerRuntime.env` (same pattern as Postgres/Redis connection strings). Bridge-mode runners get `host.docker.internal` → host gateway unless disabled (`AINATIVE_RUNNER_ADD_HOST_DOCKER_INTERNAL=false`). Optional: repo `.codex` / `.cursor` for discovery when paths exist **inside** the mounted worktree; see below. |
...
### Runner: MCP (preferred — host servers + mapping)

1. **Run MCP on the host** (or any address reachable from the task container): e.g. `npx @bytebase/dbhub` bound to `0.0.0.0:PORT` on the host.
2. **Map into the container via env** (like DB/Redis): set `Project.configJson.containerRuntime.env` with URLs pointing at the host, e.g. `PGHOST=host.docker.internal` or `MCP_DBHUB_URL=http://host.docker.internal:9xxx/sse`. Bridge runners resolve `host.docker.internal` via `--add-host=host.docker.internal:host-gateway` ([`IsolatedRunnerContainerService`](../src/containers/isolated-runner-container.service.ts)); use LAN IP or published ports if your Docker setup differs.
3. **Reference in Codex / Agent config**: use `config_overrides` or `.codex/config.toml` entries that reference those env vars or full URLs — avoid `http://127.0.0.1:...` for host-only listeners unless runner `networkMode` is `host`.
4. **Optional — cwd for repo-local discovery**: if `.codex` lives under a monorepo subfolder, still use **`runnerWorkingSubdirectory`** so `docker exec -w` matches that directory; this is orthogonal to host MCP URLs.
