...
// HTML原型生成接口
export const generateHtmlPrototype = (data: {
  prdId: string
  prdContent: string
  repository?: string
  autoCreatePr?: boolean
}) => api.post('/html-prototype/generate', data)

export const generateHtmlPrototypeFromPrd = (prdId: string, data?: {
  repository?: string
  autoCreatePr?: boolean
}) => api.post(`/html-prototype/generate-from-prd/${prdId}`, data || {})

export const getHtmlPrototypeStatus = (taskId: string) =>
  api.get(`/html-prototype/generate/${taskId}/status`)

export const getHtmlPrototypeConversation = (taskId: string) =>
  api.get(`/html-prototype/generate/${taskId}/conversation`)

export const sendHtmlPrototypeFollowup = (taskId: string, message: string) =>
  api.post(`/html-prototype/generate/${taskId}/followup`, { message })

export const stopHtmlPrototypeGeneration = (taskId: string) =>
  api.post(`/html-prototype/generate/${taskId}/stop`)

export const deleteHtmlPrototypeGeneration = (taskId: string) =>
  api.delete(`/html-prototype/generate/${taskId}`)

export const listHtmlPrototypeGenerations = (params?: { limit?: number; cursor?: string }) =>
  api.get('/html-prototype/generate', { params })
