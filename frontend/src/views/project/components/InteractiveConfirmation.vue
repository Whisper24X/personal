    ...
    <component :is="hideCard ? 'div' : 'el-card'" :class="{ 'confirmation-card': !hideCard }" :shadow="hideCard ? undefined : 'always'">
      ...
      <div class="content-section">
          ...
          <el-alert type="success" :closable="false" show-icon>
            <template #title>
              <div class="zip-info">
                ...
                <el-button type="primary" size="small" :icon="Download" @click="downloadZip"> 下载压缩包 </el-button>
              </div>
            </template>
          </el-alert>
        </div>
        ...
        <template v-else>
          ...
          <!-- Section adjustment -->
          <SectionAdjuster
            v-if="hasSectionedContent"
            :content="roleInfo.content"
      ...
      <!-- Deploy Failed Alert -->
      <el-alert v-if="isDeployFailed" type="error" :closable="false" show-icon title="部署失败" style="margin-bottom: 16px">
        <template #default>
...
const isDeployFailed = computed(() => {
  return (
    props.roleInfo.instructContent?.deployFailed === true ||
...
watch(
  () => [props.prdId, isPrototypeAction, props.roleInfo.content],
  ([, isPrototype, content]) => {
    if (isPrototype) {
...
async function confirmQuit() {
  try {
    await ElMessageBox.confirm('确定要退出吗？当前进度会被保存，您可以稍后继续。', '确认退出', {
