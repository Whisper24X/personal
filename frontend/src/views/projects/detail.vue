...
const isAutoGeneratedNodeName = (name: string): boolean => /^step-\d+$/.test(name.trim())
...
const normalizeWorkflowNodes = (nodes: WorkflowTemplateNodeForm[]) => {
  return [...nodes]
    ...
    .map((node, index) => ({
      ...node,
      nodeOrder: index + 1,
      name: !node.name.trim() || isAutoGeneratedNodeName(node.name) ? `step-${index + 1}` : node.name.trim(),
...
<template>
  <div class="fade-up w-full min-h-0 space-y-6 pb-8 md:pb-10">
    ...
    <template v-else-if="project">
      ...
      <section
        v-else-if="(workflowOnlyMode || tab === 'workflow') && canAccessWorkflow"
        class="space-y-4"
      >
        <article class="panel-card p-5">
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex items-center gap-2">
              ...
              <div v-if="canAccessWorkflow" ref="workflowAddMenuAnchorRef" class="relative">
                <button
                  type="button"
                  class="h-10 rounded-lg bg-primary px-4 text-sm font-semibold text-primary-foreground transition hover:shadow-md disabled:cursor-not-allowed disabled:opacity-60"
                  :disabled="!projectId"
