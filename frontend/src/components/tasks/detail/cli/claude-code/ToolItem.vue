...
const resultSummary = computed(() => {
  ...
  const fileName = getString(props.result.metadata?.fileName)
  const fileNumLines = props.result.metadata?.fileNumLines
  const fileTotalLines = props.result.metadata?.fileTotalLines
  if (fileName && typeof fileNumLines === 'number') {
    return `${fileName} (${fileNumLines}/${typeof fileTotalLines === 'number' ? fileTotalLines : fileNumLines} lines)`
  }
