...
export function useTaskCreatePanel(props: TaskCreatePanelProps, emit: TaskCreatePanelEmit) {
  ...
  const showGitBaseBranchSelector = computed(() => {
    return !usesImplicitGitBaseBranch(selectedProject.value)
  })
  ...
  const repositoryProvisioningHint = computed(() => {
    ...
    if (project.repositoryProvisioningStatus === 'failed') {
      ...
      return detail ? `项目仓库准备失败：${detail}` : '项目仓库准备失败，请在项目设置中重试仓库准备'
    }
    return ''
  })
  ...
  const isWorkspaceNativeProject = (project: Project | null | undefined) => {
    const configJson = project?.configJson
    if (!configJson || typeof configJson !== 'object') {
      return false
    }

    return configJson.subtreeMode === 'workspace-native' && configJson.workspaceNativeDisabled !== true
  }

  const usesImplicitGitBaseBranch = (project: Project | null | undefined) => {
    return isWorkspaceManagedProject(project) || isWorkspaceNativeProject(project)
  }
  ...
  const isTaskCreateSupportedCliToolId = (
    toolId: string,
  ): toolId is TaskCreateSupportedCliToolId => {
  ...
  const resetGitBaseBranchState = (projectDefaultBranch = '') => {
    latestBranchRequestId += 1
    loadingBranches.value = false
    branchOptions.value = []
    createForm.gitBaseBranch = projectDefaultBranch
  }
  ...
  const loadBranchesForProject = async (projectId: string) => {
    ...
    if (!projectId) {
      resetGitBaseBranchState()
      return
    }

    if (usesImplicitGitBaseBranch(project)) {
      resetGitBaseBranchState(projectDefaultBranch)
      return
    }
  ...
  const loadProjectDependencies = async (projectId: string) => {
    ...
    if (usesImplicitGitBaseBranch(project)) {
      resetGitBaseBranchState(project?.defaultBranch?.trim() || '')
    } else {
  ...
  const loadProjectsForForm = async () => {
    ...
    if (activeBusinessLineId) {
      try {
        ...
        if (workspaceProject) {
          ...
          createForm.projectId = workspaceProject.id
          return
        }
      } catch {
    ...
    if (!createForm.projectId && projects.value.length === 0) {
      try {
        const activeBlId = localStorage.getItem(STORAGE_KEYS.lastActiveBusinessLineId) ?? ''
        if (activeBlId) {
          const wsProject = await businessLinesApi.getWorkspaceProject(activeBlId)
          if (wsProject.projectId && wsProject.enabled && wsProject.name) {
            const project: Project = {
              id: wsProject.projectId,
              businessLineId: activeBlId,
              name: wsProject.name,
              slug: '_managed',
              gitUrl: '',
              defaultBranch: wsProject.defaultBranch ?? 'master',
              repositoryProvisioningStatus:
                (wsProject.repositoryProvisioningStatus as Project['repositoryProvisioningStatus']) ??
                'ready',
            }
            projects.value = [project]
            createForm.projectId = project.id
        ...
        // workspace project resolution is best-effort
      }
    }
  }
  ...
  const refreshSelectedProjectProvisioningStatus = async (projectId: string) => {
  ...
  const handleRepositoryProvisioningChanged = async (detail: { projectId: string }) => {
  ...
  const createTask = async () => {
    ...
    const projectIdForSubmit = createForm.projectId || resolveProjectIdFromContext()
    ...
    if (selected?.repositoryProvisioningStatus === 'failed') {
      ...
      showValidationError(
        detail ? `项目仓库准备失败：${detail}` : '项目仓库准备失败，请在项目设置中重试仓库准备',
      )
      return
    }
    ...
    try {
      ...
      const isWorkspaceTask = isWorkspaceManagedProject(project) && Boolean(project?.businessLineId)
      ...
      const task = await tasksApi.create({
        ...
        gitBaseBranch:
          createForm.gitBaseBranch.trim() || project?.defaultBranch?.trim() || undefined,
        configJson: {
  ...
  onMounted(() => {
    removeProjectProvisioningChangedListener = addProjectRepositoryProvisioningChangedListener(
