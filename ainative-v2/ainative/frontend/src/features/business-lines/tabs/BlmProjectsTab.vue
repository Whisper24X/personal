...
<template>
  <section class="space-y-4">
    <div class="panel-card p-4">
      <div class="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p class="text-sm font-semibold">
            项目列表<span v-if="canViewProjectList">（{{ filteredProjects.length }}）</span>
          </p>
          <p v-if="!canViewProjectList" class="mt-1 text-xs text-muted-foreground">
            当前账号暂无查看项目列表权限。
          </p>
        </div>
      ...
      <div
        v-if="!activeLineId"
        class="mt-4 rounded-xl border border-dashed border-border bg-background/70 px-4 py-5 text-sm text-muted-foreground"
      >
        请先选择业务线。
      </div>
      <div
        v-else-if="!canViewProjectList"
        class="mt-4 rounded-xl border border-dashed border-border bg-background/70 px-4 py-5 text-sm text-muted-foreground"
      >
        暂无查看项目列表权限。
      </div>
