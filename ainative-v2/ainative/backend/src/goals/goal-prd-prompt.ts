/**
 * PRD 生成：工作流风格短提示；方法论与章节约束见 `.agents/skills/prototype-to-prd/SKILL.md`（含 AINative 需求集成节）。
 */
export function buildPrdGenerationPrompt(params: {
  ...
  const inputHint = params.hasSourceDocs
    ...
    : `当前需求未关联任何输入资料文件；该目录可能为空。请仅基于需求标题与摘要推导，并在第 7 节与 uncertainPoints 中明确标注假设。`;
  ...
  return [
    '使用 prototype-to-prd 技能中的「AINative 需求 PRD 生成」约定：不要写入仓库内 PRD 文件，只向 stdout 输出 JSON。',
    '',
    `【需求标题】${params.goalTitle}`,
    params.goalSummary ? `【需求摘要】${params.goalSummary}` : '',
    `【需求 ID】${params.goalId}`,
    '',
