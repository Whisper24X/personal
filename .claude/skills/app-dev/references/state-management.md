...
| Store    | 文件           | 持久化 | 说明                                  |
| -------- | -------------- | ------ | ------------------------------------- |
| user     | userStore.ts   | ✅     | 登录状态、token、用户信息、token 管理 |
| config   | configStore.ts | ✅     | 全局配置（视频开关、客服二维码等）    |
| tabBar   | tabBarStore.ts | ❌     | TabBar 当前激活 Tab                   |
| selected | selected.ts    | —      | 选择状态                              |

## Token 管理策略（userStore）

```typescript
isTokenExpired(); // 检查 token 是否过期（服务器时间优先，兜底 12 小时本地判断）
needRefreshToken(); // 是否需要刷新 token
refreshTokenTimestamp(); // 手动延长 token 有效期
setTokenExpiredAt(expiredAt); // 保存服务器返回的过期时间
```
